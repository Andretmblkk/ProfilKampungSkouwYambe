...
use App\Filament\Resources\Demografi\PerkawinanResource\Pages;
...
class PerkawinanResource extends Resource
{
    ...
    {
        return $form->schema([
            Forms\Components\Hidden::make('kategori')->default('perkawinan'),
            Forms\Components\TextInput::make('label')->label('Status')->required(),
    ...
    {
        return $table->modifyQueryUsing(fn(Builder $query) => $query->where('kategori', 'perkawinan'))
            ->columns([
                Tables\Columns\TextColumn::make('label')->label('Status'),
    ...
    {
        return [
            'index' => Pages\ManagePerkawinan::route('/'),
        ];
    }
}
