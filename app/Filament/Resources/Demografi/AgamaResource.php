...
use App\Filament\Resources\Demografi\AgamaResource\Pages;
...
class AgamaResource extends Resource
{
    ...
    {
        return $form->schema([
            Forms\Components\Hidden::make('kategori')->default('agama'),
            Forms\Components\TextInput::make('label')->label('Agama')->required(),
    ...
    {
        return $table->modifyQueryUsing(fn(Builder $query) => $query->where('kategori', 'agama'))
            ->columns([
                Tables\Columns\TextColumn::make('label')->label('Agama'),
